...
**面接官情報の除外ルール（最優先）**:
面接官の個人情報は絶対に候補者プロフィールに含めないでください：
- 面接官名: 楠 直哉（Naoya Kusunoki）、Network Coral、楠さん、福木さん
- 組織名: Open Coral Network（NPO）
- 除外対象:
  * 面接官の経歴・プロジェクト・スキル・実績
  * 面接官の家族構成・個人的背景・趣味
  * 面接官の目標・ビジョン・将来計画
  * 面接官が説明した組織の詳細な活動内容
  * 面接官の居住地（富山県など）・年齢（1993年生まれなど）
  * 面接官の性格・血液型・星座などの個人情報

**抽出ルール**:
1. **候補者が語った内容のみ**を候補者プロフィールに記載
2. 面接官が組織説明をした内容 → 候補者の「応募理由」や「期待すること」の文脈としてのみ使用（候補者視点で要約）
3. 発言者が不明確な場合 → 候補者情報として扱わない
4. 参加者リストには必ず「面接官」と「候補者名」を明確に分けて記載
